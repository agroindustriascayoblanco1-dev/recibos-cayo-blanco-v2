...
function iniciarSistema() {
    ...
    // --------------------------------------------------
    const q1Button = document.getElementById("verQuincena1");
    const q2Button = document.getElementById("verQuincena2");
    const carnetButton = document.getElementById("verCarnet");
    const carnetModal = document.getElementById("carnetModal");
    const cerrarCarnet = document.getElementById("cerrarCarnet");

    function seleccionarServicioQuincena(valor) {
        const radio = document.querySelector(
            'input[name="quincena"][value="' + valor + '"]'
        );
        ...
        quincenaSeleccionada = valor;
        consultarEmpleado();
    }

    if (q1Button) {
        q1Button.addEventListener("click", function () {
            seleccionarServicioQuincena("q1");
        });
    }

    if (q2Button) {
        q2Button.addEventListener("click", function () {
            seleccionarServicioQuincena("q2");
        });
    }

    if (carnetButton) {
        carnetButton.addEventListener("click", function () {
            const nombre = document.getElementById("nombreEmpleado");
            const codigo = document.getElementById("codigoEmpleado");
            const carnetNombre = document.getElementById("carnetNombre");
            const carnetCodigo = document.getElementById("carnetCodigo");

            if (carnetNombre && nombre) carnetNombre.textContent = nombre.textContent;
            if (carnetCodigo && codigo) carnetCodigo.textContent = codigo.textContent;

            if (carnetModal) carnetModal.classList.remove("oculto");
        });
    }

    if (cerrarCarnet) {
        cerrarCarnet.addEventListener("click", function () {
            carnetModal.classList.add("oculto");
        });
    }

    if (carnetModal) {
        carnetModal.addEventListener("click", function (evento) {
            if (evento.target === carnetModal) {
                carnetModal.classList.add("oculto");
            }
        });
    }
