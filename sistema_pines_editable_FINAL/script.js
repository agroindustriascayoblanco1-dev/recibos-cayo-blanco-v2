...
function verificarPinAdministracion(empleado){
  ...
  const datos=window.PINES_ADMIN?.[empleado.codigo];
  if(!datos){ alert("Este empleado no tiene un PIN administrativo configurado."); return false; }
  const pinEsperado=datos.map(v=>String((Number(v)+3)%10)).join("");
